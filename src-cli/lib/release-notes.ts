...
export function getReleaseNotesPath(version: string, cwd = process.cwd()): string {
  return path.join(cwd, RELEASE_NOTES_DIRNAME, `v${normalizeVersion(version)}.md`);
}

export function getUnreleasedReleaseNotesPath(cwd = process.cwd()): string {
  return path.join(cwd, RELEASE_NOTES_DIRNAME, UNRELEASED_FILE);
}

export async function materializeVersionReleaseNotes(version: string, cwd = process.cwd()): Promise<string> {
...
export async function readReleaseNotes(version: string, cwd = process.cwd()): Promise<string> {
