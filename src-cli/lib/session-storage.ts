...
import { getSessionMode, loadConfig, type SessionMode } from "./config.js";
import { getGlobalSessionsPath } from "./paths.js";
import { listAllProfilesIncludingDuplicates, type ProfileRecord } from "./profiles.js";

export interface EnsureSessionStorageLayoutOptions {
  profileId: string;
...
export async function ensureSessionStorageForProfile(profile: ProfileRecord): Promise<void> {
...
export async function ensureSessionStorageForAllProfiles(): Promise<void> {
  const loaded = await loadConfig();
...
export async function ensureSessionStorageLayouts(optionsList: readonly EnsureSessionStorageLayoutOptions[]): Promise<void> {
  for (const options of optionsList) {
    await ensureSessionStorageLayout(options);
  }
}

export async function ensureSessionStorageLayout(options: EnsureSessionStorageLayoutOptions): Promise<void> {
...
async function ensureLocalSessionsDirectory(sessionsPath: string): Promise<void> {
  const current = await lstatSafe(sessionsPath);
  if (!current) {
...
async function mergeIntoGlobalSessions(sourceDir: string, targetDir: string, profileId: string): Promise<void> {
...
async function moveWithConflictSuffix(sourcePath: string, targetPath: string, profileId: string): Promise<void> {
