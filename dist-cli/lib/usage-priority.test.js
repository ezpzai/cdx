...
import { sortUsageRowsByPriority } from "./usage-priority.js";
...
test("1순위(정상) 안에서는 week 초기화 날짜가 가까운 순, 같은 날짜면 잔여량 높은 순으로 정렬한다", () => {
    ...
    assert.deepEqual(sorted.map((row) => row.profile), ["cdx1", "codex", "codex3", "codex7", "codex6", "codex8", "codex2", "codex4", "codex5"]);
});
