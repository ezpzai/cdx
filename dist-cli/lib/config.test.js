...
test("sessionMode가 없는 기존 config는 global로 정규화된다", async () => {
    ...
    await fsp.writeFile(configPath, `${JSON.stringify({ version: 1, defaultMode: "balanced", profiles: {}, lowQuotaPreferredProfiles: [], recentHandoffs: {} }, null, 2)}\n`, "utf8");
...
test("sessionMode가 명시된 config는 설정 완료 상태로 로드된다", async () => {
    const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "cdx-config-test-"));
    const configPath = path.join(tempDir, "config.json");
    await fsp.writeFile(configPath, `${JSON.stringify({ version: 1, defaultMode: "balanced", sessionMode: "profile", profiles: {}, lowQuotaPreferredProfiles: [], recentHandoffs: {} }, null, 2)}\n`, "utf8");
    const loaded = await loadConfig(configPath);
