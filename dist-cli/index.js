...
import { clearRecentHandoff, getGlobalDefaultMode, getLowQuotaPreferredProfiles, getProfileDefaultMode, loadConfig, rememberLowQuotaPreferredProfile, removeProfileState, RUN_MODES, saveConfig, setGlobalDefaultMode, setProfileDefaultMode, } from "./lib/config.js";
...
async function handleUsage(args) {
    ...
    for (const row of rows) {
        ...
        console.log(`  ${C}${row.profile}${R}  ${D}${account}${R}  ${D}${plan}${R}  ${W}5h${R} ${fiveHour}  ${W}week${R} ${weekly}`);
    }
    console.log("");
}
