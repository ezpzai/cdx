...
import { createProfile, getDoctorReport, prepareGlobalAgentsFile, startLoginSession, startLogoutSession, } from "./lib/actions.js";
function printHelp() {
    console.log(`cdx
...
async function chooseProfileInteractively() {
    ...
    if (profiles.length === 0) {
        throw new Error(`No profiles found. Create one with \`cdx create <name>\` or move existing homes into ${getProfilesRoot()}.`);
    }
...
async function handleUsage(args) {
    ...
    for (const row of rows) {
        ...
        const weekly = row.weeklyLeft === null ? "n/a" : `${String(row.weeklyLeft).padStart(3)}% left (${row.weeklyReset ?? "?"})`;
...
async function handleWhoAmI(profileId) {
    const profile = await requireProfile(profileId);
...
async function handleCreate(profileId) {
    if (!profileId) {
