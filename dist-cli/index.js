...
import { listProfiles, resolveProfile } from "./lib/profiles.js";
import { ensureSessionStorageForAllProfiles } from "./lib/session-storage.js";
import { confirm, prompt, selectMode, selectOne } from "./lib/terminal.js";
...
function formatResetTime(value, fallback = "?") {
    if (!value) {
        return fallback;
    }
...
function formatResetUsageTime(value, fallback = "?") {
    if (!value) {
        return fallback;
    }
    const date = new Date(value);
...
function formatColoredQuotaValue(value, resetAt) {
    const R = "\u001B[0m";
    const D = "\u001B[2m";
    if (value === null) {
        return `${D}n/a${R}`;
...
async function chooseProfileInteractively() {
    const profiles = await listProfiles();
    if (profiles.length === 0) {
        throw new Error(`No profiles found. Start one with \`cdx login <name>\` or move existing homes into ${getProfilesRoot()}.`);
    }
    const usageRows = await Promise.all(profiles.map(async (profile) => ({
        profile,
        usage: await fetchRemotePreflightUsage(profile),
        error: null,
    })));
    const usageByProfileId = new Map(usageRows.map((row) => [row.profile.id, row]));
    return selectOne("Select a profile:", profiles.map((profile) => ({
        value: profile,
        label: profile.id,
...
function formatProfileSelectionDetail(profile, row) {
    const R = "\u001B[0m";
    ...
    const fiveHourLabel = typeof fiveHour === "number" ? `${getQuotaColor(fiveHour)}${fiveHour}%${R} 5h` : `${fiveHour}% 5h`;
    const weeklyLabel = typeof weekly === "number" ? `${getQuotaColor(weekly)}${weekly}%${R} weekly` : `${weekly}% weekly`;
...
function parseRunInvocation(args) {
    const codexArgs = [];
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--mode") {
            ...
            if (!value || !RUN_MODES.includes(value)) {
...
function parseRemoteInvocation(args) {
    const codexArgs = [];
    let explicitMode = null;
    let tunnel = "cloudflare";
    let printQr = true;
...
async function pickAlternateProfile(candidates, reason) {
...
async function maybeSwitchForLowQuota(candidates) {
    const action = await selectOne("Low quota detected. Choose what to do:", [
...
function buildContinuationArgs(codexArgs, handoffPrompt) {
    return [...codexArgs, handoffPrompt];
}
function shouldCaptureRunOutput(codexArgs) {
    const command = codexArgs[0] ?? null;
    return command === "exec" || command === "review";
}
async function maybeRememberLowQuotaPreference(profileId) {
...
async function runWithProfile(profile, mode, codexArgs, cwd, config) {
    ...
    const nextConfig = clearRecentHandoff(setProfileDefaultMode(config, profile.id, getProfileDefaultMode(config, profile.id)), profile.id);
    const withHandoff = result.exitCode === 0
        ? nextConfig
        : {
            ...nextConfig,
            recentHandoffs: {
...
async function handleRun(args) {
    ...
    let candidates = [];
    let preflight = buildPreflight(activeProfile, activeMode, {
    ...
    if (preflight.quota.warning) {
        ...
        preflight = buildPreflight(activeProfile, activeMode, {
            profile: activeProfile,
            usage: currentUsage,
            error: null,
        }, candidates);
    ...
    const continuation = await runWithProfile(alternate.profile, continuationMode, continuationArgs, process.cwd(), activeConfig);
...
async function handleRemote(args) {
    ...
    printPreflight(buildPreflight(activeProfile, activeMode, {
        profile: activeProfile,
        usage,
        error: null,
    }, []));
...
async function handleRemoteDevices(args) {
...
async function handleMode(args) {
    ...
    if (subcommand === "set") {
        ...
        if (profileId) {
            ...
            config = setProfileDefaultMode(config, profile.id, mode);
            await saveConfig(config);
            console.log(`Set default mode for ${profile.id}: ${mode}`);
            return;
        }
        config = setGlobalDefaultMode(config, mode);
        await saveConfig(config);
...
function printSessionStatus(mode, globalSessionsPath) {
    const R = "\u001B[0m";
    const D = "\u001B[2m";
    const C = "\u001B[96m";
    const Y = "\u001B[93m";
    ...
    console.log(`  ${D}effect${R}   ${mode === "global"
        ? `${Y}all profiles share one sessions directory${R}`
        : `${Y}each profile uses its own sessions directory${R}`}`);
    console.log("");
}
async function ensureSessionModeConfiguredForCommand(command) {
    ...
    await promptAndPersistSessionMode({
        ...
        introductoryMessage: "Before the first Codex run, choose how sessions should be stored. Global mode merges discovered Codex homes into ~/.cdx/sessions so /resume shows one shared pool.",
    });
}
function shouldPromptForSessionMode(command, args) {
...
async function promptAndPersistSessionMode(input) {
    ...
    const nextMode = await selectOne("Choose a session storage mode", [
        {
            value: "global",
            label: "global",
    ...
    if (input.currentMode === "global" && nextMode === "profile") {
        const confirmed = await confirm("Switch to per-profile sessions? Existing globally merged sessions stay in ~/.cdx/sessions and are not auto-split.", { defaultValue: false });
        if (!confirmed) {
            console.log("Aborted.");
            return;
        }
    ...
    if (nextMode === "global") {
        const confirmed = await confirm("Use one global sessions pool? Discovered Codex homes will be scanned and merged into ~/.cdx/sessions immediately.");
        if (!confirmed) {
            console.log("Aborted.");
            return;
        }
...
async function handleLogout(profileId) {
    if (!profileId) {
